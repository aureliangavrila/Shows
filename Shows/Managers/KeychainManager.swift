...
struct KeychainManager {
    ...
    static func passwordItems(forService service: String, accessGroup: String? = nil) throws -> [KeychainManager] {
        // Build a query for all items that match the service and access group.
        var query = KeychainManager.keychainQuery(withService: service, accessGroup: accessGroup)
        ...
        var passwordItems = [KeychainManager]()
        for result in resultData {
            guard let account  = result[kSecAttrAccount as String] as? String else { throw KeychainError.unexpectedItemData }
            
            let passwordItem = KeychainManager(service: service, account: account, accessGroup: accessGroup)
